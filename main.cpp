...
// Cette fonction permet de classer la moyenne et affiche les résultats par ordre de mérite 
float Clasification()
{
    float moyenneComparaison[100]; // Tableau à manipuler pour la recherche du premier jusqu'au dernier
    float classification[100]; // Tableau dans lequel nous allons classer les résultats 
    float plusGrand,echangeur; // Les variables qui vont nous permet de rechercher à chaque fois la moyenne la plus grande 
    int cpt;
    for(i=1; i<=nbreEtudiant; i++)
    {
// Ici j'ai juste affecté les valeur du tableau de des moyenne à moyenneComparaison pour éviter de manipuler le tab des moyenne
